...
type SessionLike = { user?: { email?: string | null } } & { userId?: string };
type Tx = { _id: unknown; orderId: string; amount: number; createdAt: string | Date };

export default async function UserTransactionsById({ params }: { params: { user_id: string } }) {
  const session = await getServerSession(authOptions);
  const userId = (session as unknown as SessionLike)?.userId;
  ...
  const tx = (await db
    ...
    .find({ userId: params.user_id })
    .sort({ createdAt: -1 })
    .toArray()) as unknown as Tx[];
  return (
